...
- [ ] **数据源加载**
   - 读取 `apis.json`，优先使用其中经基准测试更新过的 `smart_payload` 和 `id_pool`。
      
- [ ] **熔断机制**
   - 针对 `DELETE` 方法，或路径中包含 `drop`, `truncate` 的接口，**强制跳过** Fuzzing，防止误删生产或测试数据。

- [ ] 异常识别：判定 biz_code == 500 且包含堆栈信息为失败。

- [ ] **多态 Fuzz 逻辑**
   - **载荷继承**：基于 `smart_payload` 变异，避免因缺少必填字段直接被拦截在 Controller 之外。
   - **JSON 场景**: 
     - 对字符串字段注入 SQL 字符 (`' OR 1=1`)。
     - 对数字字段注入超大整数 (`9999999999`)。
   - **Upload 场景**: 
     - 构造恶意文件名 (`test.jsp`, `../../config`)。
     - 上传空文件内容。
