...
- [ ] **单一变量原则**：每次只修改一个字段为边界值（如 MaxInt），其他字段保持默认合法值。

- [ ] **保护性前置检查**
   - 对于 `PUT` 或 `DELETE` 接口，若检测到目标 ID 为 `1`、`0` 或 `admin`，**直接跳过测试**，防止污染高权账号数据。
...
- [ ] **单一变量原则 (Control Variable)**
   - 每次只修改一个字段为边界值，**保持其余字段为 `smart_payload` 中的合法默认值**。
   - 这能确保 500 报错确实是由边界值引发的系统处理不当，而非参数缺失。
